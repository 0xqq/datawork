  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="任务执行CRON" prop="jobCron">
        <el-input
          v-model="queryParams.jobCron"
          placeholder="请输入任务执行CRON"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['datahub:jobinfo:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['datahub:jobinfo:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['datahub:jobinfo:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['datahub:jobinfo:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="jobinfoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="任务执行CRON" align="center" prop="id" />
      ...
      <el-table-column label="所属项目id" align="center" prop="addTime" width="180">
      ...
      <el-table-column label="任务执行超时时间，单位分钟" align="center" prop="executorTimeout" />
      ...
      <el-table-column label="调度状态：0-停止，1-运行" align="center" prop="triggerStatus" />
      ...
      <el-table-column label="动态参数" align="center" prop="replaceParam" />
      <el-table-column label="jvm参数" align="center" prop="jvmParam" />
      <el-table-column label="增量初始时间" align="center" prop="incStartTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.incStartTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="分区信息" align="center" prop="partitionInfo" />
      <el-table-column label="最近一次执行状态" align="center" prop="lastHandleCode" />
      <el-table-column label="增量时间格式" align="center" prop="replaceParamType" />
      <el-table-column label="reader表名称" align="center" prop="readerTable" />
      <el-table-column label="增量表主键" align="center" prop="primaryKey" />
      <el-table-column label="增量初始id" align="center" prop="incStartId" />
      <el-table-column label="增量类型" align="center" prop="incrementType" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="所属项目id" prop="addTime">
          <el-date-picker clearable size="small" style="width: 200px"
            v-model="form.addTime"
            type="date"
            value-format="yyyy-MM-dd"
            placeholder="选择所属项目id">
          </el-date-picker>
        </el-form-item>
        ...
        <el-form-item label="任务执行超时时间，单位分钟" prop="executorTimeout">
          <el-input v-model="form.executorTimeout" placeholder="请输入任务执行超时时间，单位分钟" />
        </el-form-item>
        ...
        <el-form-item label="调度状态：0-停止，1-运行">
          <el-radio-group v-model="form.triggerStatus">
        ...
        <el-form-item label="动态参数" prop="replaceParam">
          <el-input v-model="form.replaceParam" placeholder="请输入动态参数" />
        </el-form-item>
        <el-form-item label="jvm参数" prop="jvmParam">
          <el-input v-model="form.jvmParam" placeholder="请输入jvm参数" />
        </el-form-item>
        <el-form-item label="增量初始时间" prop="incStartTime">
          <el-date-picker clearable size="small" style="width: 200px"
            v-model="form.incStartTime"
            type="date"
            value-format="yyyy-MM-dd"
            placeholder="选择增量初始时间">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="分区信息" prop="partitionInfo">
          <el-input v-model="form.partitionInfo" placeholder="请输入分区信息" />
        </el-form-item>
        <el-form-item label="最近一次执行状态" prop="lastHandleCode">
          <el-input v-model="form.lastHandleCode" placeholder="请输入最近一次执行状态" />
        </el-form-item>
        <el-form-item label="增量时间格式" prop="replaceParamType">
          <el-select v-model="form.replaceParamType" placeholder="请选择增量时间格式">
            <el-option label="请选择字典生成" value="" />
          </el-select>
        </el-form-item>
        <el-form-item label="reader表名称" prop="readerTable">
          <el-input v-model="form.readerTable" placeholder="请输入reader表名称" />
        </el-form-item>
        <el-form-item label="增量表主键" prop="primaryKey">
          <el-input v-model="form.primaryKey" placeholder="请输入增量表主键" />
        </el-form-item>
        <el-form-item label="增量初始id" prop="incStartId">
          <el-input v-model="form.incStartId" placeholder="请输入增量初始id" />
        </el-form-item>
        <el-form-item label="增量类型" prop="incrementType">
          <el-select v-model="form.incrementType" placeholder="请选择增量类型">
            <el-option label="请选择字典生成" value="" />
          </el-select>
        </el-form-item>
...
<script>
import { listJobinfo, getJobinfo, delJobinfo, addJobinfo, updateJobinfo, exportJobinfo } from "@/api/datahub/jobinfo";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        executorTimeout: [
          { required: true, message: "任务执行超时时间，单位分钟不能为空", trigger: "blur" }
        ],
        ...
        triggerStatus: [
          { required: true, message: "调度状态：0-停止，1-运行不能为空", trigger: "blur" }
        ],
        triggerLastTime: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除作业配置编号为"' + ids + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有作业配置数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportJobinfo(queryParams);
        }).then(response => {
